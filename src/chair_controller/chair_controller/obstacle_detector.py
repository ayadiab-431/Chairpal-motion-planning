...
class ObstacleDetector(Node):
    ...
    def scan_callback(self, msg):
        # Filter ranges for valid numeric values (> 0.1m and not inf)
        valid_ranges = [(dist, idx) for idx, dist in enumerate(msg.ranges) 
                        if 0.1 < dist < float('inf')]
        
        if not valid_ranges:
            return
            
        abs_min, min_index = min(valid_ranges, key=lambda x: x[0])
        self.get_logger().info(f"DEBUG: Closest VALID obj: {abs_min:.2f}m at index {min_index}")

        # Try both 180 (front if -pi to pi) and 0 (front if 0 to 2pi)
        # We'll use the user's feedback to finalize this.
        front_indices = range(165, 196) # Cone around 180
        front_ranges = [msg.ranges[i] for i in front_indices if 0.1 < msg.ranges[i] < float('inf')]
        ...
        if min_front < self.safe_distance:
            ...
            # self.get_logger().warn(f"OBSTACLE DETECTED at {min_front:.2f} m!")
        else:
            self.obstacle_detected = False
